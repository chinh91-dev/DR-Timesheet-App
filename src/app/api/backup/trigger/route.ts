...
// UI-facing backup trigger — no CRON_SECRET required.
// This is called by the "Backup Now" and "Run Backup Now" buttons in the UI.
// The /api/cron/backup route is for Vercel's scheduled cron (requires Bearer token).
...
export async function POST(req: NextRequest) {
  ...
  if (snapshotError || !snapshot) {
    return NextResponse.json(
      { error: 'Failed to create snapshot record', detail: snapshotError?.message },
      { status: 500 }
    )
  }
